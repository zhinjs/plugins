...
sandbox.include("$", $)
async function onmessage(event:Dict) {
    if (event.post_type === "message") {
        if (event.message_type === "group" && bots.has(event.user_id) && event.user_id !== event.self_id && event.user_id < event.self_id) {
            return callApi("setGroupLeave", [event.group_id], false)
        }
        sandbox.setEnv(event)
        let res = await sandbox.run(event.cqCode)
        let echo = true
        if (event.cqCode.match(/^'\[CQ:at,qq=\d+\]'$/) && event.message_type==='private')
            echo = false
        if (res === null && event.cqCode === "null")
            echo = false
        if (["number", "boolean"].includes(typeof res) && res.toString() === event.cqCode)
            echo = false
        if (event.cqCode.substr(0,1) === "\\" && typeof res === "undefined")
            res = "<undefined>"
        res = filter(res)
        if (echo && res) {
            res = fromCqcode(res)
            if (event.message_type === "private")
                callApi("sendPrivateMsg", [event.user_id, res], false)
            else if (event.message_type === "group")
                callApi("sendGroupMsg", [event.group_id, res], false)
            else if (event.message_type === "discuss")
                callApi("sendDiscussMsg", [event.discuss_id, res], false)
        }
    } else {
        sandbox.setEnv(event)
    }
    if (!bots.has(event.self_id))
        init(event)
    else if (event.post_type === "notice" && event.notice_type === "group")
        init(event, event.group_id)
    try {
