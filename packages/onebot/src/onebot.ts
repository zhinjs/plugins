...
export class OneBot {
    ...
    constructor(protected bot: Bot, protected config: OneBotConfig = defaultOneBotConfig) {
    }
    ...
    protected _webSocketHandler(ws: WebSocket) {
        ws.on("message", async (msg) => {
            this.bot.logger.debug("OneBot - 收到ws消息：" + msg)
            var data
            try {
    ...
    async start() {
        ...
        const port=this.bot.options.plugins.http?this.bot.options.plugins!.http.port||=8086:8086
        if (!this.config.use_http && !this.config.use_ws)
            return
        if (this.config.use_http) {
            this.bot.router.all(new RegExp(`^/${this.bot.uin}/(.*)$`), this._httpRequestHandler.bind(this))
            this.bot.logger.mark(`OneBot - 开启http服务器成功，监听:http://127.0.0.1:${port}/${this.bot.uin}`)
        }
        if (this.config.use_ws) {
            this.wss = this.bot.router.ws(`/${this.bot.uin}`)
            this.bot.logger.mark(`OneBot - 开启ws服务器成功，监听:ws://127.0.0.1:${port}/${this.bot.uin}`)
