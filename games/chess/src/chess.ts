...
import {Tip} from "./tip";
import {Zhin, NSession, h, remove} from "zhin";
...
class Chess {
    ...
    // 解析棋子移动到哪儿
    public getPieceMovePos(piece: Piece, input: [string, number]): Pos {
        const [c, d] = input
    ...
    static input(message: NSession<'icqq', 'message.group'>) {
        ...
        try {
            ...
            // 走到哪
            const movePos = chess.getPieceMovePos(pickPiece, [c, d])
            ...
            // 可以走不
            const moveRes = chess.rule.checkPiecePut(pickPiece, movePos)
            // 可以走
            if (moveRes) {
                let msg: string = ''
                chess.history.push({
